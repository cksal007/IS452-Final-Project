...
# create dictionary: maps player name to their ppg
ppg_dict = {}
for player in df.values:
    ppg_dict[player[0]] = player[5]

# create dictionary: maps player name to their age
age_dict = {}
for player in df.values:
    age_dict[player[0]] = player[3]

# creating the graph for output
index = np.arange(5)
bar_width = 0.35
fig, ax = plt.subplots()
# old team bar graph
...
# adding annotations to the old team graph
for rect in old:
    ...
    ax.annotate('{}'.format(height),
                ...
                ha='center', va='bottom')
# adding annotations to the young team graph
for rect in young:
    height = rect.get_height()
    ax.annotate('{}'.format(height),
                xy=(rect.get_x() + rect.get_width() / 2, height),
...
# prompt the use for which team; loops until valid input
valid = False
while not valid:
